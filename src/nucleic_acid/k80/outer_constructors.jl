K80(κ::Float64; safe::Bool=true) = K80rel(κ, safe=safe)


K80(α::Float64, β::Float64; safe::Bool=true) = K80abs(α, β, safe=safe)


function K80(θ_vec::A;
              safe::Bool=true) where A <: AbstractArray
  if length(θ_vec) == 1
    return K80rel(θ_vec[1], safe=safe)
  elseif length(θ_vec) == 2
    return K80abs(θ_vec[1], θ_vec[2], safe=safe)
  else
